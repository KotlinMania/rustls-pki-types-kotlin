# Claude Code Project Instructions — rustls-pki-types-kotlin

## Project Overview

This is **rustls-pki-types-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rustls-pki-types`](https://crates.io/crates/rustls-pki-types).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rustls-pki-types/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rustls-pki-types>
